<?php 
  ...
  $jumlahData = count(masukkan("SELECT * FROM brng_masuk"));
  $jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
  $halamanAktif = (isset($_GET["halaman"])) ? $_GET["halaman"] : 1;
  $awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;
  
  $barangMasuk = masukkan("SELECT * FROM brng_masuk LIMIT $awalData, $jumlahDataPerhalaman");

  if(isset($_POST["cari"])) {
    $barangMasuk = cariMasuk($_POST["keyword"]);
}
?>
...
<body>
  ...
  <section>
    <div class="container">
      <h1 class="mb-3">Tampilan Barang Masuk</h1>
      ...
      <table class="table table-bordered">
        ...
        <?php $i = 1?>
        <?php foreach ($barangMasuk as $br) : ?>
        <tr>
          <td><?= $i; ?></td>
          <td><?= $br["kd_brng_masuk"] ?></td>
          <td><?= $br["kd_barang"] ?></td>
          ...
          <td><?= $br["tgl_masuk"] ?></td>
          <td>
            <a href="ubah-barang-masuk.php?km=<?= $br["kd_brng_masuk"]; ?>">Ubah</a> |
            <a href="hapus-barang-masuk.php?km=<?= $br["kd_brng_masuk"]; ?>">Hapus</a>
          </td>
        </tr>
        <?php $i++; ?>
        <?php endforeach; ?>
      ...
      <br> <br>
      <a href="input-barang-masuk.php">Tambah Barang Masuk</a>
    </div>
