<?php 
  ...
  $km = $_GET["km"];
  $br = masukkan("SELECT * FROM brng_keluar WHERE kd_brng_keluar = '$km'")[0];
  ...
  if(isset($_POST["submit"])) {
    if(ubah_keluar($_POST) > 0) {
      ...
      echo "<script>
              alert('Data Barang Keluar gagal diubah!');
              document.location.href = 'tampil-barang-keluar.php';
            </script>";
    }
  }
...
<head>
  ...
  <title>Form Ubah Barang Keluar</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-info mb-3 sticky-top">
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="navbarNavDropdown">
        <ul class="navbar-nav ms-auto">
          ...
          <li class="nav-item">
            <a class="nav-link" href="tampil-barang-keluar.php">Barang Keluar</a>
          </li>
          <li class="nav-item">
  ...
  <div class="container">
    ...
    <form action="" method="post">
      <table>
      <tr>
          <td><label for="kd_brng_keluar">Kode Barang Keluar</label></td>
          <td>:</td>
          <td><input type="text" name="kd_brng_keluar" id="kd_brng_keluar" required value="<?= $br["kd_brng_keluar"];?>"></td>
        </tr>
        <tr>
          <td><label for="kd_barang">Kode Barang</label></td>
          <td>:</td>
          <td><input type="text" name="kd_barang" id="kd_barang" required value="<?= $br["kd_barang"];?>"></td>
        </tr>
        <tr>
          <td><label for="nama_barang">Nama Barang</label></td>
          <td>:</td>
          <td><input type="text" name="nama_barang" id="nama_barang" required value="<?= $br["nama_barang"];?>"></td>
        </tr>
        <tr>
          <td><label for="jenis_barang">Jenis Barang</label></td>
          <td>:</td>
          <td><input type="text" name="jenis_barang" id="jenis_barang" required value="<?= $br["jenis_barang"];?>"></td>
        </tr>
        <tr>
          <td><label for="jumlah">Jumlah Barang</label></td>
          <td>:</td>
          <td><input type="number" name="jumlah" id="jumlah" required value="<?= $br["jumlah"];?>"></td>
        </tr>
        <tr>
          <td><label for="tgl_keluar">Tanggal Keluar</label></td>
          <td>:</td>
          <td><input type="date" name="tgl_keluar" id="tgl_keluar" value="<?= $br["tgl_keluar"];?>"></td>
        </tr>
        <tr>
    ...
    <br>
    <a href="tampil-barang-keluar.php">Lihat Item Barang</a>
  </div>
